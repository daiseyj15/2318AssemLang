...
#################################################
			.data	#all in memorey not in register
...
intArr:			.word 2, 3, 1, 8	# global int array of size 4 initialized
			...
			.globl main
main:
			la $t0, oneChar		# $t0 has address of oneChar, load address into $t0
			li $v0, 11		# prep to-print-char system service
			lb $a0, 0($t0)		# $a0 has oneChar[0] (should be 'K'), lb=load byte, (0) for right at the begining
			syscall			# should print 'K'
			ori $a0, $a0, 0x20	# lower case of char in $a0, this or imediate, the constant is 0x20 in hex the 0x tells us it is hex
			#if you actually want to change it in memory you have to store it 	
			#force lower case but , if it is lowercase and you use ori you are still lowercase
			 			#   (doesn't change oneChar[0])
			#xori $a0, $a0, 0x20	# how-it'd-be aside: toggle case of char in $a0 
			syscall			# should print 'k'
			
			sb $a0, 0($t0)		#its a store, reload to know it is from meory and not the register on line 41
			# put "char in $a0" to oneChar[0] in memory
			 			#   (need to STORE to have side effect on memory)			 			
			lb $a0, 0($t0)		# reload content of oneChar[0] in memory into $a0
			syscall			# should print 'k'
			andi $a0, $a0, 0xDF	# raise case of char in $a0, good for clearing forcing it to upercase
			syscall			# should print 'K'
			xori $a0, $a0, 0x20	# toggle case of char in $a0, making it lowercase again
			syscall			# should print 'k'
...
#################################################
			...
			#dont use $s in the begining
			#$t are good to use, k is cornell for the operationg system 
			#gp global pointer
